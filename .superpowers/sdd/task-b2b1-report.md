...
- The upload idempotency regression uses the real `screening.item.upload` operation and current `data.id` plus filename response shape. A separate historical cache row carries the file-object ID. Together they prove that no linked filename, screening-item ID, or file-object ID remains after redaction without widening the production upload response.
