...
- Important PII formatter follow-up now runs every captured worker record through the production `JsonFormatter` and injects a malicious `LogRecord` carrying a real sensitive exception traceback, pre-populated sensitive `exc_text`, and filename/object-key/candidate-text/secret/payload extras. The fully formatted JSON excludes every sensitive value while the original context-key allowlist remains enforced. Exact PII gate: `1 passed in 4.63s`; PII plus related worker gate: `5 passed in 4.90s`.
