...
from server.app.screening.terminal import finalize_llm_dead_letter,finalize_screening_dead_letter,screening_terminal_callbacks
...
def test_llm_worker_dead_letter_routes_only_complete_relational_context_once():
    ...
    with app.state.identity_store.sync_session() as db:
        ...
        queue_job=QueueRepository(db).enqueue(org.id,"screening.llm_score_item",{"organization_id":str(org.id),"screening_item_id":str(item.id),"screening_result_id":str(result.id),"application_id":str(application.id),"config_id":str(config.id),"config_version":config.version,"prompt_version_id":str(prompt.id)},dedupe_key="llm-dead",max_attempts=1)
    ...
    worker=Worker(Probe(),Probe(),interval_seconds=0,queue=DatabaseQueueGateway(url,terminal_callbacks=screening_terminal_callbacks()),handlers={"screening.llm_score_item":explode},outbox_handlers={},worker_id="llm-dead-worker",lease_seconds=30,heartbeat_seconds=10)
    ...
    with app.state.identity_store.sync_session() as db:
        ...
        assert queue_job.status=="dead_letter" and item.llm_status=="failed" and item.llm_safe_error_code=="llm_handler_failed"
