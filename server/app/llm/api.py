...
from server.app.llm.schemas import LlmConfigResource,LlmConfigUpdate,LlmTestResource
...
def _view(config,*,system,available=None):
    if config is None: return {"configured":False,"enabled":False,"provider_id":None,"model":None,"version":0,"last_test_status":None,"last_test_error_code":None,"last_test_latency_ms":None,"last_tested_at":None,**({"key_configured":False,"allowed_job_ids":[],"available_providers":available or {}} if system else {})}
    ...
    if system: data.update({"key_configured":config.encrypted_api_key is not None,"allowed_job_ids":config.allowed_job_ids,"available_providers":available or {}})
    return data
...
def get_config(request:Request):
    ...
    with request.app.state.identity_store.sync_session() as db: config=db.scalar(select(LlmProviderConfig).where(LlmProviderConfig.organization_id==principal.organization_id)); return _response(_view(config,system=_system(principal),available=request.app.state.llm_allowlist.public_view()))
...
def put_config(payload:LlmConfigUpdate,request:Request,if_match:str|None=Header(None),idempotency_key:str|None=Header(None)):
    ...
    try: request.app.state.llm_allowlist.require(payload.provider_id,payload.model)
    ...
    with request.app.state.identity_store.sync_session() as db:
        ...
        try:
            def action():
                ...
                db.flush(); db.add(AuditLog(organization_id=principal.organization_id,actor_user_id=principal.user_id,event_type="llm.config_updated",outcome="success",trace_id=request.state.trace_id,metadata_json={"config_id":str(config.id),"enabled":config.enabled})); return 200,{"data":_view(config,system=True,available=request.app.state.llm_allowlist.public_view())}
...
async def test_config(request:Request,idempotency_key:str|None=Header(None)):
    ...
    try: latency=await request.app.state.llm_gateway.test_connection(provider_id,model,api_key); status_code=200; data={"status":"succeeded","safe_error_code":None,"latency_ms":latency}
