...
export function ScheduleWorkspace({ record, candidateId, candidates, participantOptions, participantStatus, onBack, backLabel, onSave, onCheckConflicts, onGetAvailability, onNotify }) {
  ...
  return <div className="interview-page schedule-page"><button className="back-link" type="button" onClick={onBack}><ArrowLeft size={17} />{backLabel}</button>
    ...
    <div className="schedule-layout"><main className="schedule-main">
      ...
      {step === 3 && <section className="schedule-section"><header><CalendarDays size={19} /><div><h3>选择可用日期时间</h3><p>仅“可排”时段可以选择；冲突、缓冲不足、已过去和无法确认均不可选。</p></div></header><div className="schedule-slot-grid">{weekDays.map((day) => <section key={day.key}><header><strong>{day.weekday}</strong><span>{day.label}</span></header><div>{timeSlots.map((time) => { const state = getAvailabilitySlotState(availability, day.key, time, form.duration, form.timezone); return <button type="button" key={time} className={state} disabled={state !== "available" || isScheduleSlotInPast(day.key, time, form.timezone)} onClick={() => chooseSlot(day.key, time)}><strong>{time}</strong><span>{slotLabels[state]}</span></button>; })}</div></section>)}</div><footer><button className="button secondary" type="button" onClick={() => setStep(2)}>上一步</button></footer></section>}
