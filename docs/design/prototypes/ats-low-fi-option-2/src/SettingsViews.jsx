...
function OrganizationSettings({ role, onNotify, defaultTab = "成员", controller = organizationSettingsController }) {
  const editable = canEditOrganizationSettings(role);
  const state = useSyncExternalStore(controller.subscribe, controller.getSnapshot, controller.getSnapshot);
  const [tab, setTab] = useState(defaultTab === "部门" ? "部门" : "成员");
  ...
  useEffect(() => { setTab(defaultTab === "部门" ? "部门" : "成员"); }, [defaultTab]);
...
export function SettingsWorkspace({ currentRole, onRoleChange, onNotify, organizationTab = "成员" }) {
  ...
  const content = activeSection === "组织与权限" ? <OrganizationSettings role={currentRole} onNotify={onNotify} defaultTab={organizationTab} /> : activeSection === "流程与评价模板" ? <TemplateSettings role={currentRole} onNotify={onNotify} /> : activeSection === "AI 设置" ? <AiSettings role={currentRole} onNotify={onNotify} onDirtyChange={setAiDirty} /> : activeSection === "审计与数据治理" ? <AuditSettings key={currentRole} role={currentRole} onNotify={onNotify} /> : <section className="settings-denied"><LockKeyhole size={31} /><h3>无设置权限</h3><p>当前账号未获得系统设置访问权限。</p></section>;
