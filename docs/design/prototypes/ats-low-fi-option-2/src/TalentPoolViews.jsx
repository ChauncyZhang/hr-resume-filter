...
function DeferredPoolDetail({ pool, pools, memberships, candidates, positions, onBack, onUpdateMember, onMove, onRemove, onReferMember, onReactivateCandidate, onOpenCandidate, onNotify, status, error, onRetry, nextCursor, loadingMore, onLoadMore }) {
  ...
  return <div className="talent-page pool-detail-page">
    ...
    <section className="pool-detail-hero"><div><span><BriefcaseBusiness size={20} /></span><div><div><h2>{pool.name}</h2><VisibilityTag value={pool.visibility} /></div><p>{pool.purpose}</p><small>负责人：{pool.owner} · AI 初筛暂缓恢复链路</small></div></div><strong>{pool.memberCount ?? poolMembers.length}<small>人才</small></strong></section>
