...
function CandidateDetail({ candidate, onBack, onUpdate, onNotify }) {
  ...
  return <div className="candidate-page candidate-detail-page">
    ...
    <div className="candidate-detail-layout"><main className="candidate-detail-main"><section className="candidate-detail-panel"><div className="candidate-detail-tabs">{["档案与简历", "职位申请", "筛选证据", "面试与反馈", "时间线"].map((item) => <button type="button" key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item}</button>)}</div>
      ...
      {tab === "面试与反馈" && <div className="candidate-tab-content">{candidate.interviews.length ? <div className="interview-feedback-list">{candidate.interviews.map((item) => <section key={item.time}><header><div><strong>{item.round}</strong><span>{item.time}</span></div><span className="feedback-result">{item.result}</span></header><p>面试官：{item.interviewer}</p><blockquote>{item.feedback}</blockquote></section>)}</div> : <div className="candidate-empty compact"><MessageSquareText size={23} /><strong>暂无面试记录</strong><span>候选人进入面试中后，安排和反馈会显示在这里。</span></div>}</div>}
...
export function CandidatesWorkspace({ mode, setMode, selectedCandidate, setSelectedCandidate, records, setRecords, onNotify, onBackDetail }) {
  ...
  if (mode === "detail" && selectedCandidate) return <CandidateDetail candidate={selectedCandidate} onBack={onBackDetail || (() => { setSelectedCandidate(null); setMode("list"); })} onUpdate={updateCandidate} onNotify={onNotify} />;
