...
部署脚本默认根据线上版本与待发布版本的差异自动选择纯前端发布或完整发布，并且不会重复运行开发阶段已经完成的全量测试。只有在验证结果不可信或明确要求发布时重跑全量测试，才使用 `-RunFullTests`。

Nginx、TLS 和官网路由只在服务器首次初始化时配置。日常发布仅复用并校验线上已有配置，不要求再次配置，也不会重启或替换官网容器 `aurora-web`。

部署目标配置位于 `deploy/target.psd1`，不含密码或 API Key。已有服务器会执行版本化发布、健康检查和失败回滚；全新服务器会先检查 Docker、TLS 文件和企业共享网站前置条件，再生成独立密钥和初始管理员。新的 Agent 只能从仓库根目录运行 `部署到生产.ps1`，不能直接调用 `deploy/deploy-remote.ps1` 或手工修改服务器 Nginx。
